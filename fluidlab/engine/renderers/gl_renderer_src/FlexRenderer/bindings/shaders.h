...
void DrawPoints(FluidRenderBuffers *buffer, int n, int offset, float radius, float screenWidth, float screenAspect, float fov, Vec3 lightPos, Vec3 lightTarget, Matrix44 lightTransform, ShadowMap *shadowTex, Vec4 color, bool showDensity);
...
void DrawSmoke(FluidRenderer *render, GpuSmoke *s, const SmokeParticles *particles);
void DrawSmoke(FluidRenderer *render, GpuSmoke *s, const SmokeParticles *particles, const Matrix44 projMat, const Matrix44 viewMat, float radius, float screenWidth, float screenAspect, float fov);
