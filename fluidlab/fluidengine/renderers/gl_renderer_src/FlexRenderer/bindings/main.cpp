...
float g_particleRadius; // radius for rendering
...
void Init()
{
    ...
    // scene boundary planes
    (Vec4 &)g_params.planes[0] = Vec4(0.0f, 1.0f, 0.0f, 0.0f);
    (Vec4 &)g_params.planes[1] = Vec4(0.0f, 0.0f, 1.0f, 10.0f);
    (Vec4 &)g_params.planes[2] = Vec4(1.0f, 0.0f, 0.0f, 10.0f);
    (Vec4 &)g_params.planes[3] = Vec4(-1.0f, 0.0f, 0.0f, 10.0f);
    (Vec4 &)g_params.planes[4] = Vec4(0.0f, 0.0f, -1.0f, 10.0f);
    (Vec4 &)g_params.planes[5] = Vec4(0.0f, -1.0f, 0.0f, 10.0f);
...
void RenderScene()
{
    ...
    if (gpu_smoke){
        UpdateGpuSmoke(gpu_smoke, g_smoke, false);
    }
    ...
    if (g_buffers->activeIndices.size())
    {
        ...
        {
            DrawPoints(g_fluidRenderBuffers, g_bodiesNumParticles[i], g_bodiesParticleOffset[i], g_particleRadius, float(g_screenWidth), aspect, g_fov, g_lightPos, g_lightTarget, lightTransform, g_shadowMap, correct_gamma(g_bodiesColor[i]), g_drawDensity);
        }
        ...
        for (size_t i = 0; i < (size_t)g_bodiesNumParticles.size(); i++)
        {
            if (g_bodiesNeedsSmoothing[i])
            {
                RenderEllipsoids(g_fluidRenderer, g_fluidRenderBuffers, g_bodiesNumParticles[i], g_bodiesParticleOffset[i],
                             g_particleRadius, float(g_screenWidth), aspect, g_fov, g_lightPos, g_lightTarget, lightTransform,
                             g_shadowMap, g_bodiesColor[i], g_blur, g_ior, g_drawOpaque);
            }
        }

    }
    else if (g_drawPoints)
    {
        for (size_t i = 0; i < (size_t)g_bodiesNumParticles.size(); i++)
        {
            DrawPoints(g_fluidRenderBuffers, g_bodiesNumParticles[i], g_bodiesParticleOffset[i], g_particleRadius, float(g_screenWidth), aspect, g_fov, g_lightPos, g_lightTarget, lightTransform, g_shadowMap, correct_gamma(g_bodiesColor[i]), g_drawDensity);
    ...
    DrawSmoke(g_fluidRenderer, gpu_smoke, g_smoke, proj, view, g_smokeRadius, float(g_screenWidth), aspect, g_fov);
