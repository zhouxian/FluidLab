...
public:
	...
	virtual void drawSmoke(FluidRenderer *renderer, GpuSmoke *s, const SmokeParticles *particles);
	virtual void drawSmoke(FluidRenderer *renderer, GpuSmoke *s, const SmokeParticles *particles, const Matrix44 projMat, const Matrix44 viewMat, float radius, float screenWidth, float screenAspect, float fov);
	...
	virtual void drawPlanes(Vec4 *planes, int n);
	virtual void drawPoints(FluidRenderBuffers *buffers, int n, int offset, float radius, float screenWidth, float screenAspect, float fov, Vec3 lightPos, Vec3 lightTarget, Matrix44 lightTransform, ::ShadowMap *shadowTex, Vec4 color, bool showDensity);
	...
	virtual void updateGpuSmoke(GpuSmoke *s, const SmokeParticles *particles, bool updatePosition);
	virtual GpuSmoke *createGpuSmoke(const SmokeParticles *s);
