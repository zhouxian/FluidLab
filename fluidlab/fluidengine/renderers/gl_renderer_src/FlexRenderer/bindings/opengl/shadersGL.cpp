...
{
	FluidRenderBuffersGL(int numParticles = 0) : mPositionVBO(0),
												 mDensityVBO(0),
												 mIndices(0),
...
{
	DiffuseRenderBuffersGL(int numParticles = 0) : mDiffusePositionVBO(0),
...
#define USE_HDR_DIFFUSE_BLEND 0
	...
	const char *fragmentShader = STRINGIFY(
		...
		void main()
		{
			if (renderUV)
			{
				gl_FragColor = vec4(gl_TexCoord[5].xyz, 0.0f);
				return;
			}
	...
	void DrawPoints(FluidRenderBuffers *buffersIn, int n, int offset, float radius, float screenWidth, float screenAspect, float fov, Vec3 lightPos, Vec3 lightTarget, Matrix44 lightTransform, ShadowMap *shadowMap, Vec4 color, bool showDensity)
	{
		...
		if (sprogram)
		{
			...
			glVerify(glUniform4fv(glGetUniformLocation(sprogram, "colors"), 8, color));
	...
	void DrawPlanes(Vec4* planes, int n)
	{
		// diffuse 		
	...
	void DrawSmoke(FluidRenderer *renderer, GpuSmoke *s, const SmokeParticles *particles, const Matrix44 projMat, const Matrix44 viewMat, float radius, float screenWidth, float screenAspect, float fov)
	{
		if (s)
		{
			glVerify(glBindBuffer(GL_ARRAY_BUFFER, 0));
			...
			glVerify(glUseProgram(renderer->mSmokeProgram));

			glVerify(glUniform1f(glGetUniformLocation(renderer->mSmokeProgram, "pointRadius"), radius));
			glVerify(glUniform1f(glGetUniformLocation(renderer->mSmokeProgram, "pointScale"), screenWidth / screenAspect * (1.0f / (tanf(fov * 0.5f)))));

			GLuint modelViewProjectionMatrixIdx = glGetUniformLocation(renderer->mSmokeProgram, "modelViewProjectionMatrix");
			GLuint modelViewMatrixIdx = glGetUniformLocation(renderer->mSmokeProgram, "modelViewMatrix");

			glm::mat4 projMatGLM = glm::make_mat4(&projMat.columns[0][0]);
			glm::mat4 viewMatGLM = glm::make_mat4(&viewMat.columns[0][0]);
			glm::mat4 modelViewProjectionMatrix = projMatGLM * viewMatGLM;

			glUniformMatrix4fv(modelViewProjectionMatrixIdx, 1,  GL_FALSE, glm::value_ptr(modelViewProjectionMatrix));
			glUniformMatrix4fv(modelViewMatrixIdx, 1,  GL_FALSE, glm::value_ptr(viewMatGLM));

			glVerify(glBindBuffer(GL_ARRAY_BUFFER, s->mPositionsVBO));
			glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float), (void*)0);
			glEnableVertexAttribArray(1);

			glVerify(glBindBuffer(GL_ARRAY_BUFFER, s->mColorsVBO));
			glVertexAttribPointer(2, 4, GL_FLOAT, GL_FALSE, 4 * sizeof(float), (void*)0);
			glEnableVertexAttribArray(2);


			glVerify(glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, s->mIndices));
			glVerify(glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0, s->mNumParticles * sizeof(int), &particles->p_indices[0]));
			
			glVerify(glDrawElements(GL_POINTS, s->mNumParticles, GL_UNSIGNED_INT, (const void *)0));
			glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
	...
	void DrawSmoke(FluidRenderer *renderer, GpuSmoke *s, const SmokeParticles *particles)
	{
		if (s)
		{
			glEnable(GL_BLEND);  
			...
			GLuint modelMatIdx = glGetUniformLocation(renderer->mSmokeProgram, "modelMat");
			GLuint viewMatIdx = glGetUniformLocation(renderer->mSmokeProgram, "viewMat");
			GLuint projMatIdx = glGetUniformLocation(renderer->mSmokeProgram, "projMat");

			glm::mat4 projMat = glm::perspective(56.25f, 16.0f/9.0f, 0.1f, 100.0f);
			glm::mat4 viewMat = glm::lookAt(glm::vec3(1, 10, 1), glm::vec3(1, 1, 1), glm::vec3(0, 1, 0));
	...
	const char *geometryEllipsoidDepthShader =
		...
		"#extension GL_EXT_geometry_shader4 : enable\n" STRINGIFY(
			void main()
			{
				...
				if (ndcPos.x < -ndcBound)
					return;
				if (ndcPos.x > ndcBound)
					return;
				if (ndcPos.y < -ndcBound)
					return;
				if (ndcPos.y > ndcBound)
					return;
	...
	const char *vertexSmokeShader =
		"#version 330 core\n" STRINGIFY(
			...
			void main()
			{			
				gl_Position = modelViewProjectionMatrix * vec4(aPos.xyz, 1.0);
				gl_PointSize = pointRadius * (pointScale / gl_Position.w);
				ParticleColor = aColor;
			});

	const char *fragmentSmokeShader =
		"#version 330 core\n" STRINGIFY(
	...
	*/
	...
	const char *fragmentCompositeShader = STRINGIFY(
		...
		vec3 srgbToLinear(vec3 c) { return pow(c, vec3(2.2)); } vec3 linearToSrgb(vec3 c) { return pow(c, vec3(1.0 / 2.2)); }
	...
	FluidRenderer *CreateFluidRenderer(uint32_t width, uint32_t height)
	{
		...
		renderer->mSmokeProgram = CompileProgram(vertexSmokeShader, fragmentSmokeShader);
	...
	void UpdateGpuSmoke(GpuSmoke *s, const SmokeParticles *particles, bool updatePosition)
	{
		if (updatePosition) {
			auto ptr_pos = (float *)particles->p_positions_raw.request().ptr;
			glVerify(glBindBuffer(GL_ARRAY_BUFFER, s->mPositionsVBO));
			glVerify(glBufferSubData(GL_ARRAY_BUFFER, 0, s->mNumParticles * sizeof(Vec3), ptr_pos));
		}

		auto ptr_color = (float *)particles->p_colours_raw.request().ptr;
		glVerify(glBindBuffer(GL_ARRAY_BUFFER, s->mColorsVBO));
		glVerify(glBufferSubData(GL_ARRAY_BUFFER, 0, s->mNumParticles * sizeof(Vec4), ptr_color));
	}
	...
	*/
	...
	GpuSmoke *CreateGpuSmoke(const SmokeParticles *s)
	{
		GpuSmoke *smoke = new GpuSmoke();

		smoke->mNumParticles = s->particles_size;

		// vbos
		glVerify(glGenBuffers(1, &smoke->mPositionsVBO));
		glVerify(glBindBuffer(GL_ARRAY_BUFFER, smoke->mPositionsVBO));
		glVerify(glBufferData(GL_ARRAY_BUFFER, sizeof(float) * 3 * s->particles_size, &s->p_positions[0], GL_DYNAMIC_DRAW));

		glVerify(glGenBuffers(1, &smoke->mColorsVBO));
		glVerify(glBindBuffer(GL_ARRAY_BUFFER, smoke->mColorsVBO));
		glVerify(glBufferData(GL_ARRAY_BUFFER, sizeof(float) * 4 * s->particles_size, &s->p_colours[0], GL_DYNAMIC_DRAW));

		glVerify(glGenBuffers(1, &smoke->mIndices));
		glVerify(glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, smoke->mIndices));
		glVerify(glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(int) * s->particles_size, &s->p_indices[0], GL_DYNAMIC_DRAW));
...
void DemoContextOGL::drawMesh(const Mesh *m)
{
	OGL_Renderer::DrawMesh(m);
}

void DemoContextOGL::drawSmoke(FluidRenderer *renderer, GpuSmoke *s, const SmokeParticles *particles, const Matrix44 projMat, const Matrix44 viewMat, float radius, float screenWidth, float screenAspect, float fov)
{
	OGL_Renderer::DrawSmoke(renderer, s, particles, projMat, viewMat, radius, screenWidth, screenAspect, fov);
}

void DemoContextOGL::drawSmoke(FluidRenderer *renderer, GpuSmoke *s, const SmokeParticles *particles)
{
	OGL_Renderer::DrawSmoke(renderer, s, particles);
}
...
void DemoContextOGL::drawPlanes(Vec4 *planes, int n)
{
	OGL_Renderer::DrawPlanes(planes, n);
}

void DemoContextOGL::drawPoints(FluidRenderBuffers *buffers, int n, int offset, float radius, float screenWidth, float screenAspect, float fov, Vec3 lightPos, Vec3 lightTarget, Matrix44 lightTransform, ::ShadowMap *shadowTex, Vec4 color, bool showDensity)
{
	OGL_Renderer::DrawPoints(buffers, n, offset, radius, screenWidth, screenAspect, fov, lightPos, lightTarget, lightTransform, shadowTex, color, showDensity);
}
...
void DemoContextOGL::updateGpuSmoke(GpuSmoke *s, const SmokeParticles *particles, bool updatePosition)
{
	OGL_Renderer::UpdateGpuSmoke(s, particles, updatePosition);
}
...
GpuSmoke *DemoContextOGL::createGpuSmoke(const SmokeParticles *s)
{
	return OGL_Renderer::CreateGpuSmoke(s);
}
