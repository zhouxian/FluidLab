...
void CreateDemoContext(int type)
{
	...
	{
	case 0:
		context = CreateDemoContextOGL();
		break;
	case 1:
		context = CreateDemoContextD3D11();
		break;
	case 2:
		context = CreateDemoContextD3D12();
		break;
	default:
		assert(0);
	}
...
void UpdateGpuSmoke(GpuSmoke *s, const SmokeParticles *particles, bool updatePosition) {
	s_context->updateGpuSmoke(s, particles, updatePosition);
}
...
void DrawMesh(const Mesh *m) { s_context->drawMesh(m); }

void DrawSmoke(FluidRenderer *renderer, GpuSmoke *s, const SmokeParticles *particles) { s_context->drawSmoke(renderer, s, particles); }
void DrawSmoke(FluidRenderer *renderer, GpuSmoke *s, const SmokeParticles *particles, const Matrix44 projMat, const Matrix44 viewMat, float radius, float screenWidth, float screenAspect, float fov) { s_context->drawSmoke(renderer, s, particles, projMat, viewMat, radius, screenWidth, screenAspect, fov); }
...
void DrawPlanes(Vec4 *planes, int n) { s_context->drawPlanes(planes, n); }
...
{
	return s ? s_context->createGpuSmoke(s) : nullptr;
}
...
void DrawPoints(FluidRenderBuffers *buffers, int n, int offset, float radius, float screenWidth, float screenAspect, float fov, Vec3 lightPos, Vec3 lightTarget, Matrix44 lightTransform, ShadowMap *shadowTex, Vec4 color, bool showDensity)
{
	s_context->drawPoints(buffers, n, offset, radius, screenWidth, screenAspect, fov, lightPos, lightTarget, lightTransform, shadowTex, color, showDensity);
}
