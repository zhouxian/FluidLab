...
namespace
{
	float det(Vec4 v0, Vec4 v1, Vec4 v2, Vec4 v3)
	{
		...
		return v0.w * Dot(Cross(d1, d2), d3) - v1.w * Dot(Cross(d0, d2), d3) + v2.w * Dot(Cross(d0, d1), d3) - v3.w * Dot(Cross(d0, d1), d2);
	}
	...
	Vec3 intersect(Vec4 p0, Vec4 p1, Vec4 p2)
	{
		...
		Vec3 r = (p0.w * Cross(d1, d2) + p1.w * Cross(d2, d0) + p2.w * Cross(d0, d1)) / Dot(d0, Cross(d2, d1));
