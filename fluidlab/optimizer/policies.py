...
class CMAESPolicy:
    def __init__(self, init_range, action_dim, horizon, action_range, fix_dim=None):
        self.horizon = horizon
        self.action_dim = action_dim
        self.actions_v = np.random.uniform(init_range.v[0], init_range.v[1], size=(horizon, action_dim))
        self.actions_p = np.random.uniform(init_range.p[0], init_range.p[1], size=(action_dim))
        self.action_range = action_range
        self.fix_dim = fix_dim
        self.optimized_dim = list()
        for dim_idx in range(self.action_dim):
            if self.fix_dim is None or not dim_idx in self.fix_dim:
                self.optimized_dim.append(dim_idx)
        
        self.optimized_step = self.get_optimized_step()

        print('==> CMAES optimized shape : ', len(self.optimized_step), len(self.optimized_dim))
        self.es = CMAEvolutionStrategy(np.zeros([len(self.optimized_step) * len(self.optimized_dim)]), action_range[1] * 2)

    def get_optimized_step(self):
        return list(range(self.horizon))

    def sample_actions(self):
        self.action_samples = self.es.ask()
        actions = list()
        for action_sample in self.action_samples:
            # append fix dim (raw + fix --> complete)
            raw_actions = action_sample.reshape([len(self.optimized_step), len(self.optimized_dim)])
            complete_actions = self.actions_v.copy()
            for i, dim_idx in enumerate(self.optimized_dim):
                for j, step_idx in enumerate(self.optimized_step):
                    complete_actions[step_idx, dim_idx] = raw_actions[j, i]
            complete_actions = complete_actions.clip(*self.action_range)
            actions.append(np.vstack([complete_actions, self.actions_p[None, :]]))
        return actions

    def optimize(self, losses):
        self.es.tell(self.action_samples, losses)

    def get_action(self, actions, i, agent):
        return actions[i]

class LatteArtCMAESPolicy(CMAESPolicy):
    def __init__(self, *args, **kwargs):
        super(LatteArtCMAESPolicy, self).__init__(*args, **kwargs)

class LatteArtStirCMAESPolicy(CMAESPolicy):
    def __init__(self, *args, **kwargs):
        super(LatteArtStirCMAESPolicy, self).__init__(*args, **kwargs)
        
class IceCreamCMAESPolicy(CMAESPolicy):
    def __init__(self, *args, **kwargs):
        super(IceCreamCMAESPolicy, self).__init__(*args, **kwargs)

    def get_optimized_step(self):
        return list(range(169, self.horizon))

class GatheringCMAESPolicy(CMAESPolicy):
    def __init__(self, *args, **kwargs):
        self.stage_step = [50, 65, 105, 120]
        super(GatheringCMAESPolicy, self).__init__(*args, **kwargs)

    def get_optimized_step(self):
        optimized_steps = list()
        for i in range(self.horizon):
            if i % self.stage_step[3] < self.stage_step[0]:
                optimized_steps.append(i)
        return optimized_steps
    
    def get_action(self, actions, i, agent):
        if i % self.stage_step[3] < self.stage_step[0]:
            return actions[i]
        elif i % self.stage_step[3] < self.stage_step[1]:
            return np.array([0, 0.008, 0])
        elif i % self.stage_step[3] < self.stage_step[2]:
            action = (self.actions_p - agent.rigid.latest_pos.to_numpy()[0]) / (self.stage_step[2]- (i % self.stage_step[3]))
            action[1] = 0
            return action
        else:
...
class GatheringOCMAESPolicy(CMAESPolicy):
    def __init__(self, *args, **kwargs):
        self.stage_step = [50, 65, 105, 120]
        super(GatheringOCMAESPolicy, self).__init__(*args, **kwargs)
...
class CirculationCMAESPolicy(CMAESPolicy):
    def __init__(self, *args, **kwargs):
        super(CirculationCMAESPolicy, self).__init__(*args, **kwargs)